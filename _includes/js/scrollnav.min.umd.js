!function(f, h) {
    ...
    function f(f, h) {
        ...
        for (y in h)
            Object.prototype.hasOwnProperty.call(h, y) && (E[y] = h[y]);
        return E
    }
    function h(f, h) {
        if ("object" != typeof f)
        ...
        f.forEach(function(f, O) {
            ...
            E.subSections && f.matches(E.sections) && (x = y(function(f, h, y) {
                ...
                for (f = f.nextElementSibling; f && !f.matches(h); )
                    !y || f.matches(y) ? (E.push(f),
                    f = f.nextElementSibling) : f = f.nextElementSibling;
                return E
    ...
    function q(f) {
        function h() {
            ...
            return function(f, h) {
                var y = $("[data-sn-active]");
                if (f) {
                    var E = $("[data-sn-section=" + f.id + "]");
                    E && E !== y && (y && (y.removeClass("scroll-nav__item--active"),
                    y.removeAttr("data-sn-active")),
                    E.addClass("scroll-nav__item--active"),
                    E.attr("data-sn-active", !0))
                } else
                    y && (y.removeClass("scroll-nav__item--active"),
                    y.removeAttr("data-sn-active"))
            }(E, f.nav),
            E
        }
            ...
            B(h))
                if (!this.settings.insertTarget || B(this.settings.insertTarget))
                    if (["append", "prepend", "after", "before"].includes(this.settings.insertLocation)) {
                        ...
                        if (R.length)
                            return this.data = y(R, this.settings),
                            this.nav = E(this.data),
                            I = (M = this).settings.insertTarget,
                            "append" === (Q = M.settings.insertLocation) ? I.appendChild(M.nav) : "prepend" === Q ? I.insertBefore(M.nav, I.firstChild) : "before" === Q ? I.parentNode.insertBefore(M.nav, I) : "after" === Q && I.parentNode.insertBefore(M.nav, I.nextSibling),
                            O = function(f) {
                                var h = f.settings;
                                function y(y) {
                                    y.preventDefault();
                                    ...
                                    return C(L(y.target, f.data) - E, h.easingStyle).then(function() {
                                        h.updateHistory && history.replaceState({}, "", y.target.getAttribute("href")),
                            ...
                            this.settings.debug && ((F = document.createElement("div")).className = "snDebugger",
